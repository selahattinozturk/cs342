...
void *process_simulator(void *p){
    ...
    while (1){
        if (strcmp(alg, "FCFS") == 0){
            ...
            while (queues[queueIndex]->next == NULL){ // wait on the cond var
                pthread_cond_wait(&cond_var[queueIndex],&lock[queueIndex]);
            }            
            if (queues[queueIndex]->next->dummy_flag == 1 && queues[queueIndex]->burst_length == 0){ // there is dummy item in the queue
                ...
                if (mode == 2){
                    ...
                    else if (oFlag == 1){ pthread_mutex_lock(&writelock);
            ...
            if (queues[queueIndex]->next->dummy_flag == 1 && queues[queueIndex]->burst_length == 0){ // there is dummy item in the queue
                ...
                if (mode == 2){
                    ...
                    if (oFlag == 1){ pthread_mutex_lock(&writelock);
...
int main(int argc, char *argv[]){
    ...
    cond_var = (pthread_cond_t *)malloc(sizeof(pthread_cond_t) * numberOfQueues);
    ...
    if (rFlag == 0 && iFlag == 1){
        ...
        while (fscanf(ptr, "%*s %s", buf) == 1){
            ...
            if (counter % 2 == 1){
                ...
                pthread_cond_signal(&cond_var[queue_select]);
